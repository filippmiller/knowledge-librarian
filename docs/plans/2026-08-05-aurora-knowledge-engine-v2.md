...
### Задача 3.1 (ПЕРЕД 2.4, не после — циклическая зависимость из v1 плана исправлена): `CREATE EXTENSION vector`

Только расширение, без миграции колонки. Проверить доступность на текущем Railway-тарифе — если недоступно, это блокер-развилка для эскалации владельцу ДО того, как Задача 2.4 попытается создать колонку типа `vector`.
