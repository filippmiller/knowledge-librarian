...
Поле условия применимости у `Rule`/`QAPair` → таблица апостилирования из тетради → цены как таблица → замер на объёме. Без первого ошибка «260 ₽/стр вместо 1100 ₽/док» неотличима от правильного ответа: оба числа в базе есть.
