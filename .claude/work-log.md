...
## 2026-08-13 (вечер) — Прод выложен, три блокера сняты, хендофф на другую машину

Прод обновлён с отстававшей на 3 дня сборки до `888e190`, деплой `746ee24e` SUCCESS. Проверено по существу: `auroraV2Ready:true`, три таблицы `present`, `lastBootstrap.ok:true` (13/13 statements), живой вопрос отвечает `confidence: high` с цитатами из прайса.

Путь был перекрыт трижды, каждое вскрывалось после снятия предыдущего:
1. **CI на master красный с #77** — `Lint` падал на семи `no-explicit-any`; так как он стоит перед `Typecheck`/`Test`/`Build`, на master не прогонялось вообще ничего → PR #80.
2. **Первый деплой успешен и функционально пуст** — `pg_advisory_lock()` возвращает `void`, лок брали через `$queryRawUnsafe`, Prisma падала до первого DDL → параллельно починено в #82 (мой дубликат #83 закрыт).
3. **Второй деплой провалил healthcheck** — починка лока превратила быстрое падение в зависание, `register()` ждал БД без предела → PR #84 (`pg_try_advisory_lock` + предел 20с в стартовой обёртке).

Отдельно PR #81: `preserveTrigger` возвращал выдуманный триггер при простом удалении. Сообщение с другой машины проверено, дефект воспроизведён независимым падающим тестом на master. Патч на 758 строк из хендоффа НЕ применялся — база отстаёт на 25 коммитов.

Спасена незащищённая работа перед выключением машины: 3 ветки без upstream, оба стэша (как `rescue/*`), неотслеживаемый аудит из воркtree.

Остаток скоупа: перенос #70 и #52 на изменившийся `chat-provider`. Хендофф с критериями приёмки: `.claude/handoffs/2026-08-13-180000.md`.

---

## 2026-08-13 — Handoff на аудит Claude

Хендофф: `.claude/handoffs/2026-08-13-113500.md`. Задание — полный независимый аудит правок честности (source/filter/lint/eval) + арбитр по ЗАГС-документу. Прод ≠ HEAD (`39a1936` не выложен).

---

## 2026-08-13 — Арбитр: ЗАГС-инструкцию заново извлекать не надо

Открыл прод-документ `Инструкция оформление Апостиля в ЗАГСе` (55 ACTIVE). Исходник сегодня: **запись ТРЕБУЕТСЯ, неделя вперёд**. В ЛО наоборот — запись не нужна. Старый замер «НЕ ТРЕБУЕТСЯ» устарел. Существующие правила верны по фактам, раздуты дублями (~18 фактов на 55 карточек). Полный LLM-пересъём не окупается. С фильтра снят ложный маркер «неделя вперёд = ЛО» — он вырезал верный факт КЗАГС.
