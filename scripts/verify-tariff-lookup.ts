...
// Ворота на слово о цене. Измеренный дефект: «почем» входит в «ПОЧЕМУ», и
// каждый вопрос, начинающийся с «Почему…», считался ценовым. Поймано на
// корпусе: «Почему документ тождественности рекомендуют отправлять по месту
// требования как можно скорее?» — вопрос про смысл услуги, а сетка предлагала
// ответить ценой.
const NOT_PRICE: string[] = [
  'Почему документ тождественности рекомендуют отправлять по месту требования как можно скорее?',
  'Почему нотариус отказал в заверении?',
  'Что происходит на сцене приёма документов?',
];
for (const question of NOT_PRICE) {
  check(`не ценовой вопрос — ворота закрыты | ${question.slice(0, 46)}…`, !questionAsksPrice(question));
}

const PRICE: string[] = [
  'Почем нотариальное заверение?',
  'Почём апостиль?',
  'Какая цена у нотариального заверения?',
  'Сколько стоит апостиль?',
];
for (const question of PRICE) {
  check(`ценовой вопрос — ворота открыты | ${question.slice(0, 46)}`, questionAsksPrice(question));
}
