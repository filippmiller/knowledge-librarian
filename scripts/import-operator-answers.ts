 ...
 *                     текста человеком.
 *
 * Импортируемые пары получают authority-тег, то есть работают через
 * canonical-override: обходят сценарный гейт и подменяют ответ целиком с
 * уверенностью 1.0. На пяти парах риск ложного совпадения был мал, на ста
 * двадцати двух — нет, поэтому override ужесточён в той же правке: добавлен
 * порог по длинной стороне совпадения и убрано окно «200 последних пар»,
 * которое иначе вытолкнуло бы старые утверждённые эталоны.
 *
 * Запуск:
...
async function main(): Promise<void> {
  ...
  lines.push(
    apply
      ? `- импортировано без вопросов: **${toImport.length}**`
      : `- готовы к импорту без вопросов: **${toImport.length}** (запуск без --apply, ничего не записано)`
  );
