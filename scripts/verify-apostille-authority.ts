...
// Замер точности маршрутов: 10 ложных отказов из 12 на близких формулировках.
// Город доставки, получения, оплаты и назначения засчитывался местом выдачи, и
// клиент получал категорический отказ — самая дорогая ошибка из найденных.
check(
  'город ДОСТАВКИ не есть место выдачи',
  resolveApostilleTerritoriality('Нужен апостиль на справку о несудимости, отправите готовое в Казань?').kind,
  'unknown'
);
check(
  'город ПОЛУЧЕНИЯ не есть место выдачи',
  resolveApostilleTerritoriality('Апостиль на свидетельство о рождении, заберу лично в Москве').kind,
  'unknown'
);
check(
  'город КОНСУЛЬСТВА не есть место выдачи',
  resolveApostilleTerritoriality('Апостиль на справку о несудимости для консульства Италии в Москве').kind,
  'unknown'
);
check(
  'официальное название: «справка ОБ ОТСУТСТВИИ судимости» — это МВД',
  classifyApostilleDocument('Нужен апостиль на справку об отсутствии судимости'),
  'mvd'
);
