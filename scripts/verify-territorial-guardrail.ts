...
const CASES: Case[] = [
  ...
  {
    // Найдено замером на проде: ветка требовала глагола «как/можно/сделать», а
    // клиент чаще заявляет потребность. Ответ существовал, но не доходил —
    // клиент получал «уточню у коллеги» вместо продающего текста.
    name: 'ЗАЯВЛЕНИЕ О ПОТРЕБНОСТИ, а не вопрос: «нужен апостиль» тоже срабатывает',
    question: 'Справка о несудимости выдана в Саратовской области, нужен апостиль',
    expectGuardrail: true,
    mustInclude: ['Министерство внутренних дел', 'НОТАРИАЛЬНУЮ КОПИЮ'],
  },
  {
    name: 'ЗАЯВЛЕНИЕ О ПОТРЕБНОСТИ: «требуется апостиль» на диплом из другого региона',
    question: 'Требуется апостиль на диплом, выданный в Перми',
    expectGuardrail: true,
