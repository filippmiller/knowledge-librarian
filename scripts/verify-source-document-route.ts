...
const FIRES: Array<[string, 'explain_routes' | 'strict_destination']> = [
  ...
  // ВУЗ намеренно НЕ считается строгой инстанцией: подтверждённого требования
  // заверенного исходника за ним нет, а сказать «нужен заверенный исходник»
  // там, где хватает простой копии, — то же противоречие фактам, наоборот.
  ['Нужен ли оригинал для нотариального перевода диплома в ВУЗ?', 'explain_routes'],
  ['К какому документу крепится нотариальный перевод?', 'explain_routes'],
  ['Можно ли перевести нотариально по электронной копии?', 'explain_routes'],
  ['Что прикладывается к нотариальному переводу?', 'explain_routes'],
  ['Нотариальный перевод можно оформить онлайн, без визита?', 'explain_routes'],
];
...
const SILENT = [
  ...
  // Спросили цену, а не маршрут. Ветка перехватывала ценовой вопрос и отвечала
  // про исходник — найдено ревью Kimi и подтверждено измерением.
  'Сколько стоит нотариальный перевод по скану?',
  'Цена нотариального перевода, если пришлю скан?',
];
...
// Проверяется СМЫСЛ, а не формулировка: тексты переписаны языком менеджеров по
// требованию владельца, и привязка теста к конкретным словам ломала бы его при
// каждой такой правке.
check(
  'клиентский текст предлагает обойтись без приезда — это и есть заказ',
  Boolean(client && /(?:привозить|приезжать)\s+не\s+нужно|не\s+придётся\s+приезжать|отправим\s+готовое/iu.test(client.answer))
);
check(
  'клиентский текст ставит условие принимающей стороны, а не обещает безусловно',
  Boolean(client && /если\s+(?:там\s+)?не\s+требу/iu.test(client.answer))
);
