...
import { gradeCase, type ObservedCase } from '../case-grader';
...
const sourceRuleByUnitId = new Map([
  ['u4a', 4],
  ['u4b', 4],
  ['u4c', 4],
  ['u1', 1],
...
describe('gradeCase — retrieval gate идёт первым', () => {
  it('PASS, когда ожидаемое правило в top-K и всё остальное в порядке', () => {
    expect(gradeCase(expectation, observed(), sourceRuleByUnitId, { topK: 5 }).result).toBe('PASS');
  });

  it('FAIL, если ожидаемое правило вне top-K — текст ответа не спасает', () => {
    const verdict = gradeCase(
      expectation,
      observed({ rerankedUnitIds: ['u1', 'u5', 'x1', 'x2', 'x3', 'u4a'] }),
      sourceRuleByUnitId,
      { topK: 5 }
    );
  ...
  it('провал retrieval сообщается, даже если ответ безупречен', () => {
    const verdict = gradeCase(
      expectation,
      observed({ rerankedUnitIds: ['u1', 'u5'] }),
      sourceRuleByUnitId,
      { topK: 5 }
    );
...
describe('gradeCase — источник ответа', () => {
  it('general_ai — автоматический FAIL при верном тексте и найденном правиле', () => {
    const verdict = gradeCase(
      expectation,
      observed({ answerSource: 'general_ai' }),
      sourceRuleByUnitId,
      { topK: 5 }
    );
...
describe('gradeCase — совместное покрытие чисел (Q04)', () => {
  ...
  it('PASS, когда выбранные units покрывают все три числа СОВМЕСТНО', () => {
    expect(gradeCase(withNumerics, observed(), sourceRuleByUnitId, { topK: 5 }).result).toBe(
      'PASS'
    );
  });
  ...
  it('FAIL на одном осколке — покрыто 15, но не 30 и не 3 цикла', () => {
    const verdict = gradeCase(
      withNumerics,
      observed({ coveredNumerics: [{ value: 15, unit: 'seconds' }] }),
      sourceRuleByUnitId,
      { topK: 5 }
    );
...
describe('gradeCase — negative-кейсы', () => {
  ...
  it('PASS: узкое исключение найдено кандидатом, но НЕ выбрано', () => {
    expect(gradeCase(q05n1, base, rules, { topK: 5 }).result).toBe('PASS');
  });

  it('FAIL: запрещённое правило попало в выбранные', () => {
    const verdict = gradeCase(
      q05n1,
      { ...base, selectedUnitIds: ['a1', 'a3', 'a5'] },
      rules,
      { topK: 5 }
    );
  ...
  it('FAIL: обязательное правило не выбрано', () => {
    const verdict = gradeCase(q05n1, { ...base, selectedUnitIds: ['a1'] }, rules, { topK: 5 });

    expect(verdict.result).toBe('FAIL');
  });

  it('FAIL: HOLD там, где ожидался прямой ответ', () => {
    const verdict = gradeCase(q05n1, { ...base, disposition: 'HOLD' }, rules, { topK: 5 });
...
describe('gradeCase — must_clarify', () => {
  ...
  it('PASS: оба конкурирующих правила найдены, система переспросила', () => {
    expect(gradeCase(q01m1, base, rules, { topK: 5 }).result).toBe('PASS');
  });
