 ...
 *
 * The answer must sound like a self-contained human message. We deliberately
 * do NOT include source badges, document titles, rule codes, or confidence
 * labels in the user-facing text — those are internal metadata.
 *
 * Only scenario-clarification prompts keep their original shape because they
 * already contain the clean question + options.
 */
export function formatAnswerResponse(result: EnhancedAnswerResult): string {
  ...
  // Return the answer as-is so it reads like a natural human reply.
  return result.answer;
}
