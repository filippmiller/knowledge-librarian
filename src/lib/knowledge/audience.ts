...
const CONTACT_OR_SCHEDULE: Array<{ name: string; pattern: RegExp }> = [
  ...
  {
    // Номер дома пишут и без слова «дом»: «ул. Смольного 3» — самая частая
    // форма, а требование «д.»/«дом» пропускало её вовсе. Найдено замером
    // качества контролей: подложенный адрес госоргана не ловился в 123 случаях
    // из 125. Оговорка после числа обязательна — «улица переводится за 3
    // рабочих дня» адресом не является.
    name: 'адрес',
    pattern: new RegExp(
      `(?:${NOT_LETTER_BEFORE}(?:ул|пл|пр|наб)\\.|улиц[аеы]|проспект|набережн)[^.!?]{0,80}?` +
        `(?:${NOT_LETTER_BEFORE}д(?:\\.|ом)\\s*\\d|,?\\s*\\d{1,3}(?![\\d\\s]*(?:рабоч|дн|мес|год|%|руб|₽)))`,
      'iu'
    ),
  },
  {
  ...
  {
    // Часы без минут — «приём с 10 до 17», «работает с 9 до 18». Требование
    // формата ЧЧ:ММ пропускало самую живую запись времени.
    //
    // Оговорка про рабочие дни обязательна: «с 10 до 17 рабочих дней» — это
    // СРОК исполнения, а не часы приёма, и объявлять его утечкой значит глушить
    // верный ответ. Сроки в этом бюро называются постоянно.
    name: 'часы приёма без минут',
    pattern:
      /(?:при[её]м|подач[аи]|график|работа[а-яё]*)[^.!?]{0,60}?с\s*\d{1,2}\s*(?:до|—|–|-)\s*\d{1,2}(?!\s*(?:рабоч|дн|календ|недел|мес))/iu,
  },
];
