...
- Каталог triggerCondition закрыт: privacyContext = PRIVATE|PUBLIC; consentStatus = EXPLICIT|ABSENT; reachability = LIMITED|NORMAL; helperPresent = true|false; resourceAvailability = AVAILABLE|UNAVAILABLE. Условие из исходного текста, которое выражается этим каталогом, не может оставаться только прозой в statement.
...
- PRESENTATION-ONLY условие: самостоятельная условная альтернатива с условием ВНЕ закрытого trigger-каталога структурно покрыта как PROCEDURE_STEP с parentExtractionRef=null и triggerCondition=null ТОЛЬКО если (1) условие дословно сохранено в statement/evidence quote и (2) uncertainties содержит {kind:"UNRECOGNIZED_TRIGGER_CONDITION", quote:"дословная цитата условия"}. Пример: «Если так удобнее исполнителю, шаги 2 и 3 можно поменять местами» — это допустимый самостоятельный PROCEDURE_STEP (порядок необязателен, ничего не отменяет и не заменяет), если «Если так удобнее исполнителю» сохранено и явно помечено такой uncertainty; не требуй для него выдуманный trigger или parent.
- Не путай presentation-only альтернативу с настоящим override: если текст прямо отменяет, заменяет или ослабляет отдельное базовое правило (например «вместо обязательного X можно Y», «X больше не требуется»), без исполнимого каталогизированного triggerCondition это всё ещё структурный пробел. Верни POSSIBLE_OMISSION/UNREPRESENTED_CLAUSE; uncertainty не превращает неисполняемое исключение в доверенное. Например, «При отсутствии воды допустим кожный антисептик» заменяет обязательное мытьё рук с мылом — это override с исполнимым triggerCondition resourceAvailability=UNAVAILABLE, а не presentation-only альтернатива и не неисполнимый пробел.
- Проверяй отрицательную применимость: «вне дома/на людях» требует privacyContext=PUBLIC; «только после ясного согласия» — consentStatus=EXPLICIT; «если не может дотянуться» — reachability=LIMITED; «при отсутствии воды/материала» — resourceAvailability=UNAVAILABLE. Если смысл есть в statement, но structured field отсутствует, верни UNREPRESENTED_CLAUSE по дословной цитате условия.
