...
export const RESOLUTION_REASON_CODES = [
  ...
  /**
   * У исключения вообще нет `parentRuleRef`, хотя §2 делает его обязательным для
   * `EXCEPTION_RULE`. Это испорченная запись, а не «исключение без родителя»:
   * ответить им в одиночку значило бы выдать оговорку за самостоятельное
   * правило.
   */
  'exception_without_parent',
  /**
   * Ссылка на родителя есть, но самого родителя в выборке нет: он не пришёл
   * кандидатом, выбыл по CONFLICT или был заменён через `supersedes`. Отвечать
   * одним исключением нельзя по той же причине, что и при `null`, — правила,
   * к которому оно является поправкой, в ответе не будет.
   */
  'exception_parent_unavailable',
