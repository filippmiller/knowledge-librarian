...
- ВЫВОД ПО КРИТЕРИЮ, А НЕ ПО СОВПАДЕНИЮ СЛОВ. Если человек назвал конкретное обстоятельство, и описание факта выше даёт критерий, по которому это обстоятельство ОДНОЗНАЧНО отображается в одно из допустимых значений, — извлеки факт с этим значением. Не оставляй факт неизвестным лишь потому, что человек не произнёс само значение или слова из описания буквально: описание задаёт критерий решения, а не список слов для поиска. Это касается КАЖДОГО факта каталога. Если однозначного отображения по критерию нет — факт не указывай (правило «придуманное значение хуже пропущенного» остаётся главнее).
