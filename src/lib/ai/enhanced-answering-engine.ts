...
function buildSourceRouteResult(question: string): EnhancedAnswerResult | null {
  ...
  if (verdict.kind === 'strict_destination') {
    ...
    const clientAnswer = [
      'Для такой подачи исходник должен быть заверенным — подойдёт либо нотариальная копия документа, либо сам оригинал. Простой ксерокопии здесь не хватит.',
      '',
      'Значит, оригинал понадобится: с него наш нотариус снимет копию, а перевод подошьём уже к ней — или, если требуется, к самому оригиналу.',
      '',
      'Пришлите, пожалуйста, скан документа и скажите, куда именно он подаётся: подтвержу маршрут, назову стоимость и срок.',
    ].join('\n');
  ...
  const clientAnswer = [
    'Зависит от того, куда вы подаёте документ, — вариантов три.',
    '',
    'Если принимающая сторона не требует заверенного исходника, перевод подшивается к простой копии или распечатке скана. Тогда оригинал не нужен вовсе, и всё можно сделать удалённо — приезжать никуда не придётся.',
    '',
    'Если нужна подача в строгую инстанцию, исходником становится нотариальная копия: её снимает наш нотариус, и для этого оригинал нужно принести или прислать.',
    '',
    'Третий вариант — подшить перевод прямо к оригиналу, если этого требует принимающая сторона. Учтите: оригинал будет сшит с переводом навсегда.',
    '',
    'Скажите, пожалуйста, для какой страны или организации готовится документ, — подскажу нужный вариант и назову стоимость и срок. Скан или фото документа можно прислать прямо сейчас.',
  ].join('\n');
